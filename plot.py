...
def get_embeddings(experiment, data):
    ...
    torch.save((idx, mu, sigma), os.path.join(BASE_PATH, "embeddings.pt"))


def tsne():
    idx, mu, sigma = torch.load(os.path.join(BASE_PATH, "embeddings.pt"))
    x = torch.cat([mu, sigma], dim=1).detach().numpy()
    tsne = TSNE(n_components=2)
    tsne_results = tsne.fit_transform(x)
    np.save(os.path.join(BASE_PATH, "tsne.npy"), tsne_results)
...
def get_umap():
    ...
    np.save(os.path.join(BASE_PATH, "umap.npy"), embedding)
...
def plot(filter=None, interactive=True):
    ...
    umap_results = np.load(os.path.join(BASE_PATH, "umap.npy"), allow_pickle=True)
    ...
    ax = umap.plot.points(
        mapper, labels=labels, color_key_cmap="cool", subset_points=subset_points
    )
...
def generate_all_figs():
    ...
    plt.savefig(os.path.join(BASE_PATH, "all.png"))
    ...
    for h5 in tqdm(get_h5s()):
        ...
        plt.savefig(os.path.join(BASE_PATH, f"{terminal}.png"))
